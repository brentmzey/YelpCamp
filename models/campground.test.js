const { sendRequest, routes, methods, statusCodes } = require("../common/testHelpers");

describe("campgrounds routes", () => {
  it("allows a user to get all campgrounds without a search", async () => {
    const { body, statusCode } = await sendRequest(routes.campgrounds, methods.get);
  ...
  it("allows a user to search campgrounds", async () => {
    const { body, statusCode } = await sendRequest(routes.campgrounds, methods.get, { query: { search: "newport" } });
  ...
  it("redirects on 404", async () => {
    const response = await sendRequest(`${routes.campgrounds}/does-not-exist`, methods.get);
